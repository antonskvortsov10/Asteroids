    ...
    def rock_spawner(self):
        ...
        if len(self.rocks) < 10:
            ...
            def random_vel():
                ...
                :return: int or float
                '''
                return (min(2, (random.random() * (self.score / 10.0 + 0.1))) * random.choice((-1, 1)))
            ...
            for _ in range(10):  # try 10 times
                ...
                rock_vel = (random_vel(),
                            random_vel())
                rock_ang_vel = random.choice((-1, 1)) * (random.random() * 0.05 + 0.01)
...
class Asteroid(Sprite):
    ...
    def __init__(self, position, velocity,
        ...
        Sprite.__init__(self, position, velocity,
                        0, angle_velocity,
                        ('little-asteroid-' if little else 'asteroid-') + str(num))
...
def keyup(key):
    ...
    if ricerocks.started:
        if key == simplegui.KEY_MAP['left']:
            ...
            ricerocks.my_ship.turn(True if ricerocks.keydown_right else None)
        elif key == simplegui.KEY_MAP['right']:
            ricerocks.keydown_right = False
            ricerocks.my_ship.turn(False if ricerocks.keydown_left else None)
        elif key == simplegui.KEY_MAP['up']:
            ricerocks.my_ship.thrust_on_off()
        elif key == 27:  # Escape
            quit_game()
...
def switch_animate_background():
    ...
    button_animate_background.set_text('Static background' if ricerocks.animate_background_active else 'Animate background')
...
def switch_sounds():
    ...
    button_sounds.set_text('Sounds off' if ricerocks.sounds_active else 'Sounds on')
...
if __name__ == '__main__':
    ...
    frame.add_label('')
    button_animate_background = frame.add_button('Static background', switch_animate_background)
    frame.add_label('')
